...
### Timestamp formats — two separate contexts

**ffmpeg `-ss`/`-to` flags** require `HH:MM:SS.mmm`. Convert from the transcript's `MM:SS.mmm`:
- `"00:08.340"` → `"00:00:08.340"`
- `"01:30.000"` → `"00:01:30.000"`

**`transcript_with_files` timestamps** (in your JSON output) must stay in `MM:SS.mmm` format — copy them verbatim from the input transcript. Do **not** convert them to `HH:MM:SS.mmm`.
...
```json
{
  ...
  "transcript_with_files": {
    ...
    "music_cues": [
      ...
      { "id": "mc002", "file": "music/bed.mp3", "timestamps": { "start": "00:00.000", "end": "01:30.000" } }
    ]
  },
